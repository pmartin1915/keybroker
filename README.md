...
The Quickstart runs the broker through `npx tsx` for fast iteration. For a real install on a Linux host, [`examples/systemd/keybroker.service`](./examples/systemd/keybroker.service) is a paste-and-go unit file with a dedicated `keybroker` system user and the usual hardening flags (`PrivateTmp`, `ProtectSystem=strict`, `NoNewPrivileges`). The unit's header documents the full prerequisites + build-on-target + `systemctl enable` sequence. On Debian/Ubuntu you'll need `nodejs npm libsecret-1-dev` installed before the build — `keytar` dlopens libsecret at module import time, so a headless host without it crashes the broker at startup regardless of `KEYBROKER_KEYCHAIN_PATH`.
